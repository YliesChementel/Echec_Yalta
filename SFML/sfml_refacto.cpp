...
sf::Vertex createLine(const sf::Vector2f& start, const sf::Vector2f& end, sf::Color color) {
...
int main() {
    ...
    for (const auto& m : milieux) lines.push_back(createLine(center, m, sf::Color::Red));

    sf::ConvexShape losange = createLosange(center, milieux[1], milieux[5] + 0.25f * (points[1] - milieux[5]), milieux[5], sf::Color::Blue, sf::Color::Green);
    ...
    while (window.isOpen()) {
        ...
        window.clear();
        window.draw(hexagon);
        for (const auto& line : lines) window.draw(&line, 2, sf::Lines);
        window.draw(losange);
